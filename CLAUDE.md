...
IGO (Infinity Agent Orchestrator) is an Electron desktop app: a navigable infinite canvas where users spawn multiple movable/resizable terminal windows, each running a real shell via `node-pty`. Terminals are meant to launch coding agents (`codex`, `claude`) in a chosen folder.
